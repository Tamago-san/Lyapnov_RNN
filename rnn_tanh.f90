...
!        if(mod(iepo,10)==0) write(*,*) "epoch === " ,iepo
        do isample=1,sample_num
            ...
            do istep=traning_step,1,-1
                ...
                !W_out
                out_delta(istep,:)=2*(s_tr(istep,:) - s_tr_data(istep,:,isample))/dble(traning_step)
                do i=1,out_node
                do j=1,rnn_node
...
!                    drdz= 4.d0 /(( exp(z_tr(istep,i))+exp(-z_tr(istep,i)) )**2)
            ...
            enddo
            !更新
            call mean_dEdw(in_dEdw)
            call mean_dEdw(rnn_dEdw)
            call mean_dEdw(out_dEdw)
...
!    write(*,*) ""
contains
    subroutine mean_dEdw(dEdw)
        real(8) dEdw(:,:)
        dEdw(:,:) = dEdw(:,:)/dble(traning_step)
    end subroutine mean_dEdw
    function sigmoid(x)
